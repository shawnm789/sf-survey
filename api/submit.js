...
// ✅ This is the CORRECT database ID — "SF Social Survey — Responses" under Shawn's HQ
// https://www.notion.so/2ea0a3165fc3455b814b2056bda0dedd
const NOTION_DB_ID = '2ea0a3165fc3455b814b2056bda0dedd';
...
module.exports = async function handler(req, res) {
  ...
  if (!d || typeof d !== 'object') return res.status(400).json({ error: 'Missing request body' });

  // Property names and option values must match the database exactly.
  // Database: https://www.notion.so/2ea0a3165fc3455b814b2056bda0dedd
  var props = {};
  ...
  if (d.how_meet_people        && d.how_meet_people.length)   props['How They Meet People']   = msel(d.how_meet_people);
  if (d.ease_vs_other_cities)                           props['Ease vs Other Cities']   = sel(d.ease_vs_other_cities);
  if (d.friction_points        && d.friction_points.length)   props['Friction Points']        = msel(d.friction_points);
  if (d.preferred_formats      && d.preferred_formats.length) props['Preferred Formats']      = msel(d.preferred_formats);
  ...
  if (d.apps_used              && d.apps_used.length)         props['Apps Used']              = msel(d.apps_used);
  if (d.check_frequency)                                props['Check Frequency']        = sel(d.check_frequency);
  if (d.app_purpose            && d.app_purpose.length)       props['App Purpose']            = msel(d.app_purpose);
  if (d.linkedin_frequency)                             props['LinkedIn Frequency']     = sel(d.linkedin_frequency);
  if (d.linkedin_frustrations  && d.linkedin_frustrations.length) props['LinkedIn Frustrations'] = msel(d.linkedin_frustrations);
  ...
  if (d.stay_in_touch_via      && d.stay_in_touch_via.length) props['Stay in Touch Via']      = msel(d.stay_in_touch_via);
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return res.status(500).json({ error: json.message, code: json.code });
    }
